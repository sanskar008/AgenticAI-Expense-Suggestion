...
class ExpenseExtractor(Agent):
    ...
    def __init__(self):
        ...
        self.supported_formats = ['json', 'csv', 'txt', 'sms']
    ...
    def _extract_from_sms(self, input_data: str) -> List[Dict[str, Any]]:
        """Extract expenses from SMS-like text format"""
        expenses = []
        
        # Common SMS patterns
        patterns = [
            r'(?:Rs\.?|INR|₹)\s*(\d+(?:\.\d+)?)\s+(?:debited|spent|paid|withdrawn)',
            r'(?:debited|spent|paid|withdrawn)\s+(?:Rs\.?|INR|₹)\s*(\d+(?:\.\d+)?)',
            r'(\d+(?:\.\d+)?)\s*(?:debited|spent|paid|withdrawn)',
        ]
